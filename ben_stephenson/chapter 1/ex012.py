...
latitude1, longitude1 = [radians(int(i)) for i in input('Введите широту и долготу первой точки через пробел: ').split()]
latitude2, longitude2 = [radians(int(i)) for i in input('Введите широту и долготу второй точки через пробел: ').split()]
distance = (6371.01 * acos(sin(latitude1) * sin(latitude2) +
            cos(latitude1) * cos(latitude2) * cos(longitude1 - longitude2)))
print(f'Расстояние между двумя точками {distance: .3f} км')
